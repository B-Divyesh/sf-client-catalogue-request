...
- Deployed with `/opt/fleet/lib/deploy-container.sh client-catalogue-request /work/repo Dockerfile 8080`. Container App revision `sf-client-catalogue-request--bu9moj9` runs `sociobotregistry.azurecr.io/sf-client-catalogue-request:6e903c2b43f2`; its managed certificate is SNI-bound to `https://client-catalogue-request.sociobot.in`.
- Live identity verification on 2026-08-28: `/opt/fleet/lib/verify-url.sh https://client-catalogue-request.sociobot.in /tmp/ccr-repair-public-evidence` passed (HTTP 200, 601 ms browser load, no console errors, title, `lang=en`, exactly one h1, main landmark, and zero missing image alt text). `GET /health` returned `{"build_sha":"6e903c2b43f245a138855e466637bf29f6112008","ok":true}`.
