...
- Storage namespace: `demo:client-catalogue-request:requests` stores the random workspace ID and `demo:client-catalogue-request:submitted` stores submitted sample requests. Demo API calls are stateless and never read, write, or retain seller or demo data on the server.
